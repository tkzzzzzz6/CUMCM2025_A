...
use_random_coarse = false;   % true: 随机均匀采样；false: 规则网格
...
% 为了画整段航迹，取从 0 到 best.te 的时间
tU = linspace(0, best.te, max(2, ceil(best.te/dt_plot)));
U  = U0 + (tU(:) .* best.v) .* h; U(:,3)=U0(3);

% XY 俯视
figure('Color','w','Position',[60 80 900 650]); hold on; grid on; axis equal;
plot(M(:,1), M(:,2), '-', 'LineWidth',1.2);
plot(U(:,1), U(:,2), '-', 'LineWidth',1.2);
plot(M0(1),M0(2),'^','MarkerSize',8,'MarkerFaceColor','auto');
plot(U0(1),U0(2),'o','MarkerSize',8,'MarkerFaceColor','auto');
plot(best.E(1),best.E(2),'ks','MarkerSize',9,'LineWidth',1.2);  % 起爆点
% 真目标脚印
tt = linspace(0,2*pi,200);
plot(real_r*cos(tt), 200 + real_r*sin(tt), 'k-', 'LineWidth',1.6);
% 在遮蔽中点时刻画云团投影圈
if any(mask)
    tidx = round(mean(find(mask)));
    th2 = linspace(0,2*pi,120);
    plot(C(tidx,1)+R_cloud*cos(th2), C(tidx,2)+R_cloud*sin(th2), '--', 'LineWidth',1.2);
end
plot(decoy(1),decoy(2),'x','MarkerSize',9,'LineWidth',1.2);
xlabel('X (m)'); ylabel('Y (m)');
title('XY 俯视：最优方案航迹与起爆点');
legend({'M1 轨迹','FY1 轨迹','M1 起点','FY1 起点','起爆点 E','真目标脚印','云团投影(遮蔽中点)','诱饵'}, ...
