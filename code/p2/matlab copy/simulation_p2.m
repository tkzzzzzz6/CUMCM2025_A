...
% 预计算遮蔽度量（距离与判据）
R = R_cloud;
d = nan(N,1); tau = nan(N,1); mask = false(N,1);
for k = find(idxCloud).'
    Ck = C(k,:); Mk = M(k,:);
    AB = Tpt - Mk;
    denom = dot(AB,AB);
    if denom < eps
        tau(k) = 0; d(k) = norm(Ck - Mk);
    else
        tau_un = dot(Ck - Mk, AB) / denom;
        tau(k) = tau_un;
        tclamp = min(max(tau_un,0),1);
        Q = Mk + tclamp * AB;
        d(k) = norm(Ck - Q);
    end
    mask(k) = (d(k) <= R) && (tau(k) >= 0) && (tau(k) <= 1);
end
...
% 视频输出
saveGIF = true; saveMP4 = true; gifName = 'problem2_anim.gif'; mp4Name = 'problem2_anim.mp4'; fps = 25;
if saveMP4
...
for k = 1:N
    ...
    set(txtInfo, 'String', sprintf('t = %.2f s\n云团球心距LOS: %s\n遮蔽中: %s\n累计遮蔽: %.2f s', t(k), dstr, occ, cum_occ));
